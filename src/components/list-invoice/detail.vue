    ...
    <div v-if="invoice">
      ...
      <div class="card customer-info">
        ...
        <div><strong>Trạng thái khách hàng:</strong>
          <input v-model="invoice.khachHang.trangThaiKhachHang" type="checkbox" :checked="invoice.khachHang.trangThaiKhachHang === 1" />
        </div>
      </div>
        ...
        <div v-if="invoice.chiTietHoaDonList.length > 0">
          <ul>
            <li v-for="(item, index) in invoice.chiTietHoaDonList" :key="index">
              ...
              <input v-model="item.maGhe" /> <br>
              <strong>Giá tiền:</strong>
              <input v-model="item.giaTien" type="number" /> <br>
              <strong>Phòng chiếu:</strong>
              <input v-model="item.phongChieu" />
            </li>
          </ul>
        </div>
        <div v-else>
      ...
      <div class="button-container">
        ...
        <button @click="saveInvoice">Lưu lại</button>
      </div>
    </div>
    <div v-else>
...
export default {
  ...
  computed: {
    // Invoice details (maGhe, giaTien, phongChieu) computed properties
    maGhe: {
      get() {
        return this.invoice?.chiTietHoaDonList[0]?.id_GheAndSuatChieu?.ghe?.maGhe || '';
      },
      set(value) {
        if (this.invoice?.chiTietHoaDonList[0]?.id_GheAndSuatChieu?.ghe) {
          this.invoice.chiTietHoaDonList[0].id_GheAndSuatChieu.ghe.maGhe = value;
        }
      }
    },
    giaTien: {
      get() {
        return this.invoice?.chiTietHoaDonList[0]?.id_GheAndSuatChieu?.ghe?.giaTien || '';
      },
      set(value) {
        if (this.invoice?.chiTietHoaDonList[0]?.id_GheAndSuatChieu?.ghe) {
          this.invoice.chiTietHoaDonList[0].id_GheAndSuatChieu.ghe.giaTien = value;
        }
      }
    },
    phongChieu: {
      get() {
        return this.invoice?.chiTietHoaDonList[0]?.id_GheAndSuatChieu?.ghe?.phongChieu?.maPhong || '';
      },
      set(value) {
        if (this.invoice?.chiTietHoaDonList[0]?.id_GheAndSuatChieu?.ghe) {
          this.invoice.chiTietHoaDonList[0].id_GheAndSuatChieu.ghe.phongChieu.maPhong = value;
        }
      }
    }
  }
